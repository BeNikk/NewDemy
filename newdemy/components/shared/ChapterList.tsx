...
export const ChapterList=({items,onReorder,onEdit}:ChapterListInterface)=>{
    ...
    function onDragEnd(result:DropResult){
        ...
        const bulkUpdateData=updatedChapters.map((chapter)=>(
            {id:chapter.id,position:items.findIndex((item)=>item.id==chapter.id)}
        ))
                            ...
                            {(provided)=>(
                                <div className={cn("flex items-center gap-x-2 bg-slate-200 border-slate-200 border text-slate-700 rounded-md mb-4 text-sm",chapter.isPublished && "bg-sky-100 border-sky-200 text-sky-700")} ref={provided.innerRef} {...provided.draggableProps}>
                                    ...
                                    <div className="ml-auto pr-2 flex items-center gap-x-2">
                                        ...
                                        <Pencil className="size-4 cursor-pointer hover:opacity-75 transition" onClick={()=>{onEdit(chapter.id)}}/>
